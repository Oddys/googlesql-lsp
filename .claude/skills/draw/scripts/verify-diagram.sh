...
total = len(box) + len(glob) + len(path) + len(act)
if total == 0:
    print('OK — no layout issues found (checked text-in-box overflow, viewBox-width overflow, arrow/text collisions, activation-bar/arrow alignment)')
    sys.exit(0)
print('LAYOUT ISSUES FOUND: %d box overflow, %d viewBox overflow, %d arrow/text collisions, %d activation-bar misalignments' % (len(box), len(glob), len(path), len(act)))
print()
