...
act = report.get('activationIssues', [])
cross = report.get('crossingIssues', [])
total = len(box) + len(glob) + len(path) + len(act) + len(cross)
if total == 0:
    print('OK — no layout issues found (checked text-in-box overflow, viewBox-width overflow, arrow/text collisions, activation-bar/arrow alignment, activation-bar crossings)')
    sys.exit(0)
print('LAYOUT ISSUES FOUND: %d box overflow, %d viewBox overflow, %d arrow/text collisions, %d activation-bar misalignments, %d activation-bar crossings' % (len(box), len(glob), len(path), len(act), len(cross)))
print()
