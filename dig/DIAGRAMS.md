...
- **`language_name` gets dropped crossing the ABI**: it's available at the host wrapper (`extension_lsp_adapter.rs`, `wasm_host.rs:88-93`) but is never forwarded into the actual guest call — `language_server_command` in `zed-extension/src/lib.rs` only ever receives the language-server ID and a worktree handle (`wit.rs:225-238`).
